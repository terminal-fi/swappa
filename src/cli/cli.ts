...
async function main() {
  ...
  for (const route of routes.slice(0, 5)) {
    ...
    console.info(
      `Output: ${route.outputAmount
        ...
        .toFixed(6)} -- ` + `${path.join(":")}:${outputToken.symbol}`
    );
  }
  ...
  if (from && routes.length > 0) {
    ...
    if (inputAmount.gt(allowance)) {
      const approveTX = inputTKN.methods.approve(
        manager.routerAddr,
        inputAmount.toFixed(0)
      );
      console.info(`Sending approve TX...`);
      const approveReceipt = await approveTX.send({
        from: from,
      });
    ...
    const tx = manager.swap(
      route,
      inputAmount,
      route.outputAmount.multipliedBy(opts.maxSlippage),
      from,
      { precheckOutputAmount: precheck }
    );
    console.info(`Sending TX...`);
    const receipt = await tx.send({ from: from });
