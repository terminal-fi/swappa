  ...
  for (let d = 0; d < maxSwaps; d += 1) {
    ...
    for (const route of currentRoutes.values()) {
      ...
      for (const pair of matchingPairs) {
        const outputT =
          pair.tokenA === route.outputToken
            ? pair.tokenB
            : pair.tokenB === route.outputToken
            ? pair.tokenA
            : null;
        if (!outputT) {
          throw new Error(
            `pairsByToken is invalid? ${pair.tokenA}/${pair.tokenB} !== ${route.outputToken}`
          );
        }
        if (
          pair.pairKey !== null &&
          route.pairs.find((p) => p.pairKey === pair.pairKey)
        ) {
          continue; // skip already used or conflicting pairs.
        }
        const outputTAmount = pair.outputAmount(
          route.outputToken,
          route.outputAmount
        );
        const maxOutputAmount =
          maxOutputAmounts.get(outputT) || new BigNumber(0);
