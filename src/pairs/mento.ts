...
export class PairMento extends PairXYeqK {
	...
	public async refresh(): Promise<void> {
		...
		this.refreshBuckets(new BigNumber(1).minus(spread), buckets.bucketCELO, buckets.bucketSTB)
	}
	...
	private mentoBucketsAfterUpdate = async () => {
		...
		const bucketCELO = reserveGoldBalance.multipliedBy(reserveFraction).integerValue(BigNumber.ROUND_DOWN)
		const bucketSTB = bucketCELO.multipliedBy(oracleRate.rate).integerValue(BigNumber.ROUND_DOWN)
